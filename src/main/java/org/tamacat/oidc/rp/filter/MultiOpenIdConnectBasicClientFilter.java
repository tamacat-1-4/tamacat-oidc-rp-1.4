...
public class MultiOpenIdConnectBasicClientFilter implements Filter {

	static Log LOG = LogFactory.getLog(MultiOpenIdConnectBasicClientFilter.class);
	...
	protected Map<String, OpenIdConnectConfig> openIdConnectConfig;
	...
	public MultiOpenIdConnectBasicClientFilter() {
		this.openIdConnectConfig = new OpenIdConnectConfigLoader().load("openid-connect.json");
	}
	...
	public void setOpenIdConnectConfig(String file) {
		openIdConnectConfig = new OpenIdConnectConfigLoader().load(file);
	}

	protected String getClientId(HttpServletRequest req) {
		String id = getId(req);
		return openIdConnectConfig.get(id).getClientId();
	}
	...
	public void doFilter(HttpServletRequest req, HttpServletResponse resp) {
		...
		if (session) {
			...
			if (uri.startsWith(req.getContextPath()+"/oidc")) {
				String id = getId(req);
		...
		if (StringUtils.isNotEmpty(authHeader)) {
			...
			if (credentials.length == 2 && "Bearer".equals(credentials[0])) {
				...
				try {
					...
					String id = getId(req);
					RSAPublicKey publicKey = getPublicKey(openIdConnectConfig.get(id).getJwksUri(), kid);
					if (publicKey == null) {
	...
	protected void handleLogoutRequest(HttpServletRequest req, HttpServletResponse resp) {
		String id = getId(req);
		String logoutUri = openIdConnectConfig.get(id).getEndSessionEndpoint();
		String redirectUri = getOriginURL(req, openIdConnectConfig.get(id).getRedirectUri());
	...
	protected String getUrlforCodeFlowAuth(HttpServletRequest req, String redirectUri) {
		...
		AuthorizationCodeRequestUrl codeUrl = new AuthorizationCodeRequestUrl(
			openIdConnectConfig.get(id).getAuthorizationEndpoint(), clientId)
	...
	public TokenResponse getTokenResponse(HttpServletRequest req, String code, String nonce) {
		...
		AuthorizationCodeTokenRequest2 tokenUrl = new AuthorizationCodeTokenRequest2(
			httpTransport, jsonFactory, new GenericUrl(openIdConnectConfig.get(id).getTokenEndpoint()), code);
		String clientId = getClientId(req);
		String redirectUri = getOriginURL(req, openIdConnectConfig.get(id).getCallbackUri());
		
		tokenUrl.setGrantType("authorization_code");
		tokenUrl.setRedirectUri(redirectUri);
		tokenUrl.set("client_id", clientId);
		tokenUrl.set("client_secret", openIdConnectConfig.get(id).getClientSecret());
